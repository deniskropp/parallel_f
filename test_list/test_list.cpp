// === (C) 2020/2021 === parallel_f / test_list (tasks, queues, lists in
...
int main() {
  ...
  auto func = [](auto a) {
    parallel_f::log_info("Function %s\n", a.c_str());

    std::this_thread::sleep_for(std::chrono::milliseconds(100));

    parallel_f::log_info("Function %s done.\n", a.c_str());
  };

  std::vector<std::shared_ptr<parallel_f::core::task_base>> tasks;

  for (int i = 0; i < 17; i++)
    tasks.push_back(parallel_f::make_task(func, std::string("running task") +
                                                    std::to_string(i) + "..."));
