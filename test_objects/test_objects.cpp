// === (C) 2020 === parallel_f / test_objects (tasks, queues, lists in parallel
...
public:
  ...
  entity() : id(next_id()), x(0), y(0), seq(0) {}
};

class funcs {
public:
  static constexpr auto run = [](entity *o) {
    parallel_f::log_info("object::funcs::run(%llu)...\n", o->id);

    std::this_thread::sleep_for(std::chrono::milliseconds(10));

    o->seq++;

    parallel_f::log_info("object::funcs::run(%llu) done.\n", o->id);
  };

  static constexpr auto show = [](entity *o) {
    parallel_f::log_info("object id %llu, x %d, y %d, seq %d\n", o->id, o->x,
                         o->y, o->seq);
  };
};
...
int main() {
  ...
  for (int i = 0; i < 4; i++) {
    for (int n = 0; n < objects.size(); n++) {
      ...
      auto run_id =
          tl.append(parallel_f::make_task(object::funcs::run, o), flush_id);
