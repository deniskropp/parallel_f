// === (C) 2020/2021 === parallel_f / test_pause (tasks, queues, lists in
...
protected:
  virtual bool run() {
    parallel_f::log_info("TestTask::run()...\n");

    thread = new std::thread([this]() {
      parallel_f::log_info("  <- TestTask::run thread...\n");

      std::this_thread::sleep_for(std::chrono::milliseconds(500));

      enter_state(task_state::FINISHED);

      parallel_f::log_info("  <- TestTask::run thread done.\n");
    });

    parallel_f::log_info("TestTask::run() done.\n");
