...
//	parallel_f::setDebugLevel("vthread::", 1);
//	parallel_f::setDebugLevel("vthread::run", 1);
//	parallel_f::setDebugLevel("vthread::join", 1);
//	parallel_f::setDebugLevel("vthread::manager::", 1);
//	parallel_f::setDebugLevel("task::", 1);
//	parallel_f::setDebugLevel("task_base::", 1);
...
//	parallel_f::setDebugLevel("task_cl::kernel_exec::kernel_exec", 1);
//	parallel_f::setDebugLevel("OCL_Device", 1);
//	parallel_f::setDebugLevel("Grafx::main", 1);
...
Grafx::Grafx()
	:
	window(sf::VideoMode(1000, 700), "Grafx"),
	frames(0),
	frames_first(0),
	update(true)//,
//	joinables(2)
{
	...
	args = task_cl::make_args(new task_cl::kernel_args::kernel_arg_mem(image.size() * sizeof(uint32_t), NULL, image.data()),
...
bool Grafx::loop(unsigned int iterations)
{
	while (iterations-- && window.isOpen()) {
		...
		if (clock.getElapsedTime().asSeconds() >= 1.0f) {
			parallel_f::logLineF("FPS ", (frames - frames_first) / clock.restart().asSeconds());
			frames_first = frames;
